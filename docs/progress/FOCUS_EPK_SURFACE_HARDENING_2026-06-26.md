...
- Middleware uses `EPK_ACCESS_PASSWORD` from Cloudflare environment variables; no password is committed to GitHub.
...
- EPK deployment requires Cloudflare Pages environment variables for protected/private behavior, especially `EPK_ACCESS_PASSWORD` for the password gate.
...
- Redeploy/test EPK on Cloudflare with required environment variables set.
...
- Review and merge Beam branch `beam/ai-change-review-queue-v2` into Beam `main` if staged updates are acceptable.
