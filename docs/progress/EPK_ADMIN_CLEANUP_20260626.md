...
- `devknowsdev/EPK#14` — `feat: add EPK block canvas admin`
...
- `devknowsdev/EPK#15` — `docs: package and validate EPK admin upgrade`
  ...
  - No admin/app code changes in cleanup PR.
...
Publishing remains an explicit external-write action. The admin may prepare, preview, download, or explicitly publish JSON, but should not silently mutate public data or perform external writes without deliberate user action.

## Validation

EPK now includes this static validator:
