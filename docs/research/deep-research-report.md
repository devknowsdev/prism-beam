...
| `prism-beam` | Workspace meta repo with smoke-test/helper scripts and coordination docs; at research time, helper scripts referenced sibling repos named `ADHDashboard-git` and `AI-Forge`, which indicated legacy naming still existed in tooling citeturn41view0turn34view1 | Workspace-level scripts and templates | Current Prism naming/docs, local stack bootstrap, shared check scripts | **Workspace coordination repo** | No runtime logic | Update layout/docs/scripts to current repo names and local stack | Do not turn it into another product repo |
